...
import NodePropertiesSheet from "@/components/main/node_editor/dialogsAndMenus/NodePropertiesSheet.tsx";
import ContextMenu from "@/components/main/node_editor/dialogsAndMenus/NodeRightClickMenu.tsx";
import UtilityBar from "@/components/main/node_editor/dialogsAndMenus/utilityBar.tsx";
import {useUndoRedo} from "@/components/main/node_editor/useUndoRedo.ts";
import EdgePropertiesSheet from "@/components/main/node_editor/dialogsAndMenus/EdgePropertiesSheet.tsx";
...
const NodeEditor = ({panelSize}: { panelSize: number }) => {
    ...
    const updateEdgeData = useCallback((id: string, patch: Partial<EdgeData>) => {
        ...
        setSelectedEdge((prev) => (prev?.id === id ? {
            ...prev,
    ...
    const onAutoCreate = useCallback(() => {
        ...
        // Map generated edges into a Set for fast lookup to exclude them from physical creation
        const generatedEdgeKeys = new Set(
            (generatedEdges || []).map(([a, b]) => [a, b].sort((a,b) => a.localeCompare(b)).join('::'))
        );

        const newEdges: Edge<EdgeData>[] = [];
        edgePairs.forEach(([a, b]) => {
            // Skip this edge if it is dynamically generated by the protocol (e.g. swap)
            const edgeKey = [a, b].sort((a,b) => a.localeCompare(b)).join('::');
            if (generatedEdgeKeys.has(edgeKey)) return;
    ...
    return (
        <div id="node-editor-whole-container" className="h-full w-full flex flex-col gap-3 p-4 pb-0 pt-3 bg-card rounded-lg">
            ...
            <div className="relative shrink-0 flex flex-col items-center pt-2">
                <Button
                    onClick={() => {
                        setShowNetworkCapacity((prev) => !prev)
                        setTimeout(() => fitView(), 50)
                    }}
                    ...
                    title={showNetworkCapacity ? "Hide Network Capacity" : "Show Network Capacity"}
                >
                    {showNetworkCapacity ? (
